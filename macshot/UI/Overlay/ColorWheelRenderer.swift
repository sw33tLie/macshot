...
class ColorWheelRenderer {
    ...
    private let swatchRadius: CGFloat = 14
    let colors: [NSColor] = [
    ...
    func draw(currentColor: NSColor) {
        ...
        for (i, color) in colors.enumerated() {
            ...
            let r = isHovered ? swatchRadius + 4 : swatchRadius
        ...
        let centerR: CGFloat = 12
        let centerRect = NSRect(x: center.x - centerR, y: center.y - centerR, width: centerR * 2, height: centerR * 2)
        currentColor.setFill()
        NSBezierPath(ovalIn: centerRect).fill()
        NSColor.white.withAlphaComponent(0.6).setStroke()
        NSBezierPath(ovalIn: centerRect.insetBy(dx: 0.5, dy: 0.5)).lineWidth = 1.5
        NSBezierPath(ovalIn: centerRect.insetBy(dx: 0.5, dy: 0.5)).stroke()
    }
